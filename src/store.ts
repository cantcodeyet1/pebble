...
import { getTodayLOTD } from './db/wotd'
...
interface PebbleStore {
  ...
  recordActivity: () => void
  addLogos: (
    logos: Omit<Logos, 'id' | 'dateAdded' | 'nextReviewDate' | 'masteryLevel'>
  ) => void
...
export const usePebbleStore = create<PebbleStore>(set => ({
  ...
  setActivityDates: dates =>
    set({ activityDates: dates, streak: computeStreak(dates) }),
  recordActivity: () =>
    set(state => {
      const today = toDateStr(new Date())
  ...
  appendLogos: logos =>
    set(state => {
      ...
      return {
        logoi: [logos, ...state.logoi],
        activityDates: newDates,
        streak: computeStreak(newDates)
      }
    }),
  removeLogos: id =>
    set(state => ({ logoi: state.logoi.filter(l => l.id !== id) })),
  addLogos: newLogos =>
    set(state => ({
      logoi: [
        ...state.logoi,
  ...
  updateMastery: (id, success) =>
    set(state => ({
      logoi: state.logoi.map(l => {
        if (l.id === id) {
          const newLevel = success
            ? Math.min(l.masteryLevel + 1, 5)
            : Math.max(l.masteryLevel - 1, 0)
          return {
            ...l,
            masteryLevel: newLevel,
            nextReviewDate: addDays(new Date(), Math.pow(2, newLevel)) //leitner
          }
        }
        return l
      })
