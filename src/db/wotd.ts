...
export async function getTodayWotd(): Promise<WotdEntry | null> {
  const conn = getConn();
  const today = new Date().toISOString().slice(0, 10);
  const result = await conn.query('SELECT * FROM wotd_log WHERE date = ?', [today]);
...
export async function saveWotd(entry: WotdEntry): Promise<void> {
  const conn = getConn();
  await conn.run(
    'INSERT OR REPLACE INTO wotd_log (date, word_data, phrase_data) VALUES (?, ?, ?)',
    [entry.date, JSON.stringify(entry.word), JSON.stringify(entry.phrase)],
  );
}
